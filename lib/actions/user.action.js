...
export async function getUser(userId) {
  try {
    ...
    const user = await User.findById(userId).populate(
      "friends",
      "_id name username profileAvatarIndex"
    );
    if (!user) {
    ...
    // Convert all friends' _id to string
    if (plainUser.friends && Array.isArray(plainUser.friends)) {
      plainUser.friends = plainUser.friends.map((friend) => ({
        ...friend,
