...
export async function createOrGetUserProgress(chapterId, userId) {
  try {
    await dbConnect();
    ...
    if (existingProgress) {
      ...
      const plainProgress = existingProgress.toObject();
    ...
    // Convert the Mongoose document to a plain object and handle special fields
    const plainProgress = newProgress.toObject();
  ...
  try {
    ...
    if (chapterId === 6) {
      const { success } = await addNewAchievement(
        userId,
        achievements.CURRICULUM_COMPLETION
      );
