...
export async function getAllScores({ game, gameMode }) {
  try {
    ...
    const leaderboard = await Leaderboard.findOne({
      ...
      select: "name username profileAvatarIndex",
    }); // Only get name, username, profileAvatarIndex and _id
    ...
    const result = leaderboard
      ? {
          ...
          topScores: leaderboard.topScores.map(({ userId, score }) => ({
            ...
            profileAvatarIndex: userId.profileAvatarIndex, // Keep profileAvatarIndex
            score,
          })),
        }
      : null;
