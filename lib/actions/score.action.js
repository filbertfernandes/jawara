...
export async function updateScore({ userId, game, gameMode, score }) {
  try {
    ...
    console.log(
      "DEBUG: Checking if this is the first time playing the game..."
    );
    const isFirstTime =
    ...
    if (isFirstTime) {
      console.log(
        `DEBUG: First time playing ${game}, checking for achievements...`
      );
      ...
      if (achievementId) {
        console.log(
          `DEBUG: Assigning first-time achievement: ${achievementId}`
        );
    ...
    console.log(
      `DEBUG: Checking if the new score should be updated for ${game}, mode: ${gameMode}`
    );
    if (
      ...
      console.log(`DEBUG: Updating score to ${score}`);
      user.scores[game][gameMode] = score;
      await user.save();

      console.log(
        `DEBUG: Fetching leaderboard for game: ${game}, mode: ${gameMode}`
      );
      const leaderboard = await Leaderboard.findOne({ game, gameMode });

      if (!leaderboard) {
        console.log("DEBUG: No leaderboard found, creating a new one...");
      ...
      console.log("DEBUG: Checking if user is already in the leaderboard...");
      const existingEntryIndex = leaderboard.topScores.findIndex(
        (entry) => entry.userId.toString() === user._id.toString()
      );

      if (existingEntryIndex !== -1) {
        console.log("DEBUG: User already in leaderboard, updating score...");
        leaderboard.topScores[existingEntryIndex].score = score;
      } else {
        console.log("DEBUG: Checking if user qualifies for the leaderboard...");
        if (leaderboard.topScores.length < 100) {
          ...
          if (
            ...
            console.log("DEBUG: User qualifies, updating leaderboard...");
      ...
      console.log("DEBUG: Sorting leaderboard...");
      if (game === "game1") {
      ...
      console.log("DEBUG: Checking for leaderboard achievements...");
      ...
      for (const { rank, achievement } of leaderboardAchievements) {
        if (userRank < rank) {
          console.log(
            `DEBUG: Assigning leaderboard achievement: ${achievement}`
          );
    ...
    console.log("DEBUG: Score update completed successfully.");
    return {
      success: true,
