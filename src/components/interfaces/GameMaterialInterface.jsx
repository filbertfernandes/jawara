...
const WordVariant = ({ variant, label, playSound }) => (
    <div className="flex justify-between text-base mb-2 lg:text-xl">
        <div>
            {variant} <span className="text-[0.5rem] text-white/50 lg:text-xs">({label})</span>
        </div>
                ...
                {words.map((word, index) => (
                    <div key={index} className="flex justify-between items-center w-full h-40 bg-stone-800/50 rounded-[1rem] mb-2 pl-4 sm:w-[45%] xl:w-[30%] xl:h-44">
                        ...
                        <div className="flex flex-col w-full h-full px-4 py-2 xl:py-4">
                            ...
                            <WordVariant variant={word.ngoko} label="Ngoko" playSound={handlePlaySound} />
                            <WordVariant variant={word.madya} label="Krama Madya" playSound={handlePlaySound} />
                            <WordVariant variant={word.alus} label="Krama Alus" playSound={handlePlaySound} />
                        </div>
                    </div>
                ))}
            </div>
