...
const GameMenuInterface = ({ startGame, title }) => {
    ...
    return (
        <div className={ `flex flex-col justify-center items-center gap-6 w-full h-[75%] sm:h-full ${isVisible ? 'animate-bounceIn' : 'opacity-0'}` }>
            <h1 className="text-4xl text-sky-400 drop-shadow-lg font-bold">{ title }</h1>

            <button 
                className="p-1 bg-stone-800/50 w-28 text-sm text-white font-semibold rounded-lg shadow-md"
