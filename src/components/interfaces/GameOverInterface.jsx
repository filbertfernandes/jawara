const GameOverInterface = ({ score, startGame, goToMenu }) => {
    ...
    return (
        <div className="flex flex-col justify-center items-center gap-6 w-full h-[90%]">
            ...
            <button className='p-1 bg-stone-800/50 w-28 text-sm text-white font-semibold rounded-lg shadow-md lg:p-1.5 lg:w-52 lg:text-3xl' onClick={ startGame } onKeyDown={ handleKeyDown } >Retry</button>
            <button className='p-1 bg-stone-800/50 w-28 text-sm text-white font-semibold rounded-lg shadow-md lg:p-1.5 lg:w-52 lg:text-3xl' onClick={ goToMenu } onKeyDown={ handleKeyDown } >Back to Menu</button>
        </div>
    )
}
